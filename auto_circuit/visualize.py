...
def net_viz(
    ...
    for e in seq_edges:
        ...
        values.append(0.8 if prune_scores is None else (abs(edge_score) if use_abs else edge_score))
...
def draw_seq_graph(
    ...
    if seq_len > 1:
        ...
        for patch_mask in edge_scores:
            ps_seq_tots = t.clamp(patch_mask.abs() if use_abs else patch_mask - score_threshold, min=0.0)
